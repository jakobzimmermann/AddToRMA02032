...
Module ReclaimAsNew
    ' ReclaimAsNew - reads a list of serial numbers from Todd's file and update to 'RestockAsNew'
    '
    ...
    Sub Main()
        ...
        Console.WriteLine("Starting to Reclaim As New from Todd's list on " & Today.UtcNow.ToString & "(UTC)")
        ...
        End Try
        Console.WriteLine("Completed Reclaim As New from Todd's list on " & Today.UtcNow.ToString & "(UTC)")
        ...
        If System.IO.File.Exists(SerialNumberFile) = True Then
            ...
            Do While SNReader.Peek() <> -1
                ...
                Try
                    ...
                    SQLstring = "Update prod_head set packing_list_number='RESTOCKNEW' where serial_number='" & SN & "'"
                    myCommand1.CommandText = SQLstring
                    myCommand1.ExecuteNonQuery()
                    SQLstring = "Update repair_head set return_notes='Reclaim for new stock' " & _
                        ", return_as_new_stock=1 where serial_number='" & SN & "' and rma_number='RMA02073'"
                    myCommand1.CommandText = SQLstring
                    myCommand1.ExecuteNonQuery()
